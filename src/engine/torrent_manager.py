...
class TorrentManager:
    ...
    def __init__(self):
        ...
        try:
            ...
            logging.info(
                "Connected to qBittorrent at %s:%s", QBITTORRENT_HOST, QBITTORRENT_PORT
            )
            # Secure the download directory
            save_path = self._client.app.defaultSavePath()
            if save_path:
                setup_secure_dir(save_path)
            ...
            if login_failed and isinstance(e, login_failed):
                ...
                raise TorrentConnectionError(
                    "שגיאת התחברות ל-qBittorrent. בדוק שם משתמש וסיסמה."
                ) from e
    ...
    def _unregister_torrent(user_id: int):
        ...
        with _lock:
            if user_id in _active_torrents:
                ...
                logging.info(
                    "Unregistered torrent %s for user %s. Global count: %d",
                    torrent_hash[:8],
                    user_id,
                    _global_active_count,
                )
        ...
        try:
            ...
            if isinstance(source, str) and source.startswith("magnet:"):
                ...
                if result == "Ok.":
                    ...
                    if torrent_hash and self._client.torrents_info(
                        torrent_hashes=torrent_hash
                    ):
                        logging.info(
                            "Torrent %s already exists (add returned Fails)",
                            torrent_hash,
                        )
                    else:
                        raise TorrentError(
    ...
    def is_complete(self, torrent_hash: str) -> bool:
        ...
        return (
            status.get("state", "").endswith("UP") or status.get("progress", 0) >= 100
        )
    ...
    def remove_torrent(
        self, torrent_hash: str, user_id: int, delete_files: bool = True
    ):
        """
        ...
        try:
            ...
            logging.info(
                "Removed torrent %s (delete_files=%s)", torrent_hash[:8], delete_files
            )
        except Exception as e:
