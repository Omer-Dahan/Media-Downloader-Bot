...
from engine.archive_manager import split_file
...
class TorrentDownload(BaseDownloader):
    ...
    def _poll_progress(self) -> bool:
        ...
        if status.get("state") == "error" or status.get("state") == "missingFiles":
            raise TorrentError(
                f"שגיאת טורנט: {status.get('error', status.get('state'))}"
            )
    ...
    def _start(self):
        ...
        try:
            ...
            if has_video:
                ...
                try:
                    largest_video = max(
                        detected_videos, key=lambda p: Path(p).stat().st_size
                    )
                    vid_meta = self._extract_video_metadata(Path(largest_video))
                    if vid_meta:
                        torrent_meta.update(vid_meta)
                        logging.info(
                            "Extracted metadata for largest torrent video: %s",
                            {k: v for k, v in vid_meta.items() if k != "thumb"},
                        )
                except Exception as e:
            ...
            if self._manager and self._torrent_hash:
                try:
                    self._manager.remove_torrent(
                        self._torrent_hash, self._user_id, delete_files=True
                    )
                except Exception as e:
