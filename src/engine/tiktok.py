...
try:
    import gallery_dl as _  # noqa: F401 — import check only, module unused directly
...
class TikTokDownload(BaseDownloader):
    ...
    def _download_with_ytdlp_url(self, url: str) -> list:
        ...
        if TIKTOK_COOKIES_FILE and pathlib.Path(TIKTOK_COOKIES_FILE).exists():
            logging.info(
                "TikTok: Using cookies file for yt-dlp: %s", TIKTOK_COOKIES_FILE
            )
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if info:
                    ...
                    logging.info(
                        "TikTok DEBUG: title len=%d, description len=%d, fulltitle len=%d",
                        len(title_field),
                        len(desc_field),
                        len(fulltitle_field),
                    )
                    ...
                    if title:
                        ...
                        logging.info(
                            "TikTok: Using longest title (%d chars): %s",
                            len(title),
                            title[:100],
                        )
                    else:
    ...
    def _download_slideshow_with_gallery_dl(
        self, url: str
        ...
        try:
            ...
            if TIKTOK_COOKIES_FILE and pathlib.Path(TIKTOK_COOKIES_FILE).exists():
                logging.info(
                    "TikTok: Using cookies file for gallery-dl: %s", TIKTOK_COOKIES_FILE
                )
            ...
            if image_files:
                logging.info(
                    "TikTok: gallery-dl found %d images and audio=%s",
                    len(image_files),
                    bool(audio_file),
                )
        ...
        try:
            ...
            with TikTokAPI() as api:
                ...
                if video_bytes:
                    # Save to temp file
                    title = (
                        getattr(video, "desc", "tiktok_video")[:70] or "tiktok_video"
                    )
    ...
    def _start(self):
        ...
        if self._is_slideshow and self._slideshow_images:
            logging.info(
                "TikTok: Uploading slideshow with %d images",
                len(self._slideshow_images),
            )
            ...
            if len(self._slideshow_images) > 1:
                ...
                for i, img_path in enumerate(
                    self._slideshow_images[:10]
                ):  # Telegram limit: 10 items
                    if i == 0:
                        media_group.append(
                            types.InputMediaPhoto(media=img_path, caption=caption)
                        )
                    else:
            ...
            if success:
                ...
                remaining_text = (
                    f" | קרדיטים נותרים: {self._remaining_credits}"
                    if self._remaining_credits
                    else ""
                )
