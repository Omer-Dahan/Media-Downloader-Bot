...
def create_zip(source_path: Path, output_dir: Path = None) -> Path:
    ...
    try:
        ...
        logging.info("Created ZIP archive: %s (%.1f MB)", zip_path, zip_size / (1024 * 1024))
        return zip_path
...
def split_file(file_path: Path, output_dir: Path = None, 
               part_size: int = 1900 * 1024 * 1024) -> list[Path]:
    """
    ...
    logging.info("Splitting file %s (%d MB) into %d MB parts", 
                base_name, file_size // (1024*1024), part_size // (1024*1024))
...
def create_split_archive(source_path: Path, output_dir: Path = None, 
                         part_size: int = 1900 * 1024 * 1024) -> list[Path]:
    """
