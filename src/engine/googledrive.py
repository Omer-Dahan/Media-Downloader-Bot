...
class GoogleDriveDownload(BaseDownloader):
    ...
    def _get_doc_type(self) -> str | None:
        ...
        if "docs.google.com/document" in self._url:
            return "document"
        if (
            "docs.google.com/spreadsheets" in self._url
            or "sheets.google.com" in self._url
        ):
            return "spreadsheets"
        if (
            "docs.google.com/presentation" in self._url
            or "slides.google.com" in self._url
        ):
            return "presentation"
        return None
    ...
    def _download_with_curl(self, file_id: str, output_dir: Path) -> Path | None:
        ...
        try:
            ...
            if "text/html" in content_type and len(response.content) < 50000:
                ...
                if (
                    "error" in content_preview.lower()
                    or "denied" in content_preview.lower()
                ):
            ...
            utf8_match = re.search(
                r"filename\*=UTF-8''([^;\s]+)", content_disp, re.IGNORECASE
            )
            if utf8_match:
            ...
            logging.info(
                "curl_cffi download successful: %s (%d bytes)",
                filename,
    ...
    def _download(self, formats=None) -> list:
        ...
        logging.info(
            "Google download complete: %s (%.1f MB)", output_file.name, file_size_mb
        )
