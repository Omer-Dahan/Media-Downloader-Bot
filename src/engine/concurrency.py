...
class ConcurrencyManager:
    ...
    def acquire(self, user_id: int) -> bool:
        ...
        with self._lock:
            ...
            if current >= limit:
                logging.warning("User %s rejected: %d/%d active tasks", user_id, current, limit)
                return False
    ...
    def release(self, user_id: int):
        ...
        with self._lock:
            ...
            if current > 0:
                ...
                logging.info("User %s released slot, remaining: %d", user_id, self._active_tasks.get(user_id, 0))
            else:
                logging.warning("User %s tried to release slot but has 0 active tasks", user_id)
