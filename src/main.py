...
from engine import (
    direct_entrance,
    youtube_entrance,
    youtube_entrance_with_quality,
    get_youtube_video_info,
    special_download_entrance,
    torrent_entrance,
    is_magnet_link,
    jdownloader_entrance,
    TORRENT_AVAILABLE,
)
...
def report_error_to_archive(
    client: Client, user: types.User, url: str, error: Exception | str
):
    import html
...
def private_use(func):
    def wrapper(client: Client, message: types.Message):
        ...
        # message type check
        if message.chat.type != enums.ChatType.PRIVATE and not getattr(
            message, "text", ""
        ).lower().startswith("/ytdl"):
...
def help_handler(client: Client, message: types.Message):
    ...
    client.send_message(
        chat_id, BotText.help, disable_web_page_preview=True, reply_markup=markup
    )
...
def ping_handler(client: Client, message: types.Message):
    ...
    def send_message_and_measure_ping():
        ...
        client.edit_message_text(
            chat_id=reply.chat.id, message_id=reply.id, text="בדיקת הפינג הושלמה."
        )
        time.sleep(1)
...
def direct_download(client: Client, message: types.Message):
    ...
    url, _ = extract_url_and_name(message_text)
    ...
    try:
        ...
        if "בוטלה" not in str(e):
            logging.info(
                "Direct download failed for %s, falling back to JDownloader2: %s",
                url,
                e,
            )
            try:
                ...
                [
                    [
                        types.InlineKeyboardButton(
                            "לרכישת קרדיטים 💬", url="https://t.me/YD_IL"
                        )
                    ]
                ]
            ),
...
def download_handler(client: Client, message: types.Message):
    ...
    if is_magnet_link(text):
        ...
        bot_msg = message.reply_text(
            "🧲 מגנט לינק התקבל, מתחיל הורדת טורנט...", quote=True
        )
    ...
    try:
            ...
            markup = types.InlineKeyboardMarkup(
                [
                    [  # First row - high quality
                        types.InlineKeyboardButton(
                            "🎬 1080p HD", callback_data=f"ytq:1080:{url_hash}"
                        ),
                        types.InlineKeyboardButton(
                            "🎬 720p", callback_data=f"ytq:720:{url_hash}"
                        ),
                    ],
                    [  # Second row - lower quality
                        types.InlineKeyboardButton(
                            "🎬 480p", callback_data=f"ytq:480:{url_hash}"
                        ),
                        types.InlineKeyboardButton(
                            "🎬 360p", callback_data=f"ytq:360:{url_hash}"
                        ),
                    ],
                    [  # Third row - audio only
                        types.InlineKeyboardButton(
                            "🎵 שמע בלבד", callback_data=f"ytq:audio:{url_hash}"
                        ),
                    ],
                ]
            )
        ...
        bot_msg: types.Message | Any = message.reply_text(
            "הקישור נקלט, מתחיל לעבוד עליו 📥", quote=True
        )
        client.send_chat_action(chat_id, enums.ChatAction.UPLOAD_VIDEO)
        ...
        f.write(
            b"\xd7\x94\xd7\x91\xd7\xa7\xd7\xa9\xd7\x94 \xd7\xa9\xd7\x9c\xd7\x9a \xd7\x91\xd7\x98\xd7\x99\xd7\xa4\xd7\x95\xd7\x9c. \xd7\x90\xd7\xa0\xd7\x90 \xd7\x94\xd7\x9e\xd7\xaa\xd7\x9f \xd7\x91\xd7\xa1\xd7\x91\xd7\x9c\xd7\xa0\xd7\x95\xd7\xaa."
        )
        f.name = "אנא המתן.txt"
        message.reply_document(
            f, caption=f"המתנת הצפה! אנא המתן {e} שניות...", quote=True
        )
        f.close()
...
def _build_settings_markup(chat_id):
    ...
    title_length = get_title_length_settings(
        chat_id
    )  # returns 100, 250, 500, 1000, 4000, 0
    ...
    title_btn = types.InlineKeyboardButton(
        f"📑 אורך תיאור: {title_len_display}", callback_data="toggle_title_len"
    )
...
def toggle_quality_callback(client: Client, callback_query: types.CallbackQuery):
    ...
    try:
        callback_query.message.edit_text(
            BotText.settings, reply_markup=_build_settings_markup(chat_id)
        )
...
def toggle_long_desc_callback(client: Client, callback_query: types.CallbackQuery):
    ...
    callback_query.answer(
        "הגדרה זו הוחלפה בהגדרת 'אורך תיאור' (4,000 או ללא הגבלה).", show_alert=True
    )
...
def youtube_quality_callback(client: Client, callback_query: types.CallbackQuery):
    ...
    if quality == "audio":
        ...
        callback_query.answer(
            f"⏳ מתחיל הורדה באיכות {quality_names.get(quality, quality)}..."
        )
    ...
    try:
        if quality == "audio":
            ...
            callback_query.message.edit_text(
                f"🔄 מוריד באיכות {quality_names.get(quality, quality)}..."
            )
    except Exception as e:
    ...
    try:
        ...
        if download_type == "direct":
            ...
            if quality and quality not in ["high", "medium", "low"]:
                ...
                youtube_entrance_with_quality(
                    client, callback_query.message, url, quality
                )
            else:
...
By @BennyThink, VIP Mode: {ENABLE_VIP} 
    ...
    def send_update_notification_on_startup():
        time.sleep(3)  # Wait for bot to fully connect
        try:
        ...
        try:
            ...
            if JDOWNLOADER_EMAIL:
                ...
                logging.info(
                    "ℹ️ JDownloader2 not configured (JDOWNLOADER_EMAIL is empty)"
                )
